...
{
	double angle = get_degreses(degrees - rotate_circle_offset);
	int x1 = width / 2;
	int y1 = width / 2;
...
{
	double hours_angle = clock.get_minutes() * 30;
...
void ClockGraphic::draw_hours_text()
{
	for (int i = 0; i < 60; i++)
	{
		...
		if (i < 3 || i > 8)
		{
			length = 100; //substitute fix for offset of the numbers
		}
		...
		std::unique_ptr<Shape> text = std::make_unique<Text>(device_context, std::to_string(hour), Point(x, y));
		text->draw();
	}
}
